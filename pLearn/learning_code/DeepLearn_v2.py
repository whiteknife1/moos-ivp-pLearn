...
class Deep_Learner:
    ...
    def initialize(self):
        ...
        if self.alg_type == "A/C":
            ...
            print("-------------INITIALIZING------------------")
            self.state_inputs, self.action_inputs, self.actor_model, self.critic_model = self.init_actor_critic()
            _, _, self.target_actor_model, self.target_critic_model = self.init_actor_critic()
            # set actor gradients
            self.actor_critic_grad = tf.placeholder(tf.float32, [None, 1])
            actor_model_weights = self.actor_model.trainable_weights
            actor_grads = tf.gradients(self.actor_model.output, actor_model_weights, -self.actor_critic_grad)
            grads = zip(actor_grads, actor_model_weights)
            self.optimize = tf.train.AdamOptimizer(self.lr).apply_gradients(grads)
            # Set critic gradients
            critic_grads = tf.gradients(self.critic_model.output, self.action_inputs)
...
##################################################################################################################
    ...
    def train_actor_critic(self, data):
        ...
        for d in data:
            ...
            # actor model requires state as input and outputs action
            target_action = self.target_actor_model.predict(self.state2vec(new_state))
            # critic model requires state action pair as input and outputs reward
            # takes the new state and the suggested action from the actor model as input
            future_reward = self.target_critic_model.predict([self.state2vec(new_state), self.state2vec(target_action)])[0][0]
            reward += self.discount_factor*future_reward
            print("------------------Array Lengths---------------------")
            print(str(self.state2vec(cur_state).shape))
            print(str(self.state2vec(action).shape))
            ...
            grads = self.sess.run(self.critic_grads, feed_dict = {
                self.state_inputs: self.state2vec(cur_state),
                self.action_inputs: self.state2vec(predicted_action)
            })[0]
    ...
    """
    Procedure: init_actor_critic()
    Purpose:
    Returns: a neural network for the actor model and the critic model
    """

    def init_actor_critic(self):
        if not self.load:
            state_inputs = Input(shape=(Constants.num_states+1,))
            action_inputs = Input(shape=(None, 1)) # action array is [1, 2] which goes to (None, 3)
            state_1 = Dense(units=self.num_units, activation='relu')(state_inputs)
            state_2 = Dense(units=self.num_units*2, activation='relu')(state_1)
            action_1 = Dense(units=self.num_units, activation='relu')(action_inputs)
            action_2 = Dense(units=self.num_units*2, activation='relu')(action_1)
            merge = Add()([state_2, action_2])
            merge_1 = Dense(units=self.num_units, activation='relu')(merge)
            actor_output = Dense(1, activation='linear')(state_2)
            critic_output = Dense(1, activation='linear')(merge_1)
            # create actor
            optimizer = Adam(lr=self.lr)
            actor = Model(inputs=state_inputs, output=[actor_output])
            actor.compile(optimizer=optimizer, loss = "mse")
            # create critic
            critic = Model(inputs=[state_inputs, action_inputs], output=[critic_output])
            critic.compile(optimizer=optimizer, loss="mse")
        # load models
        else:
            actor = load_model(Constants.load_model_dir+"actor.h5")
            critic = load_model(Constants.load_model_dir+"critic.h5")
        actor.save(self.save_dir+"actor.h5")
        critic.save(self.save_dir+"critic.h5")
        return (state_inputs, action_inputs, actor, critic)
