...
class Deep_Learner:
    ...
    def initialize(self):
        ...
        if self.alg_type == "A/C":
            ...
            self.actor_model self.critic_model = self.init_actor_critic()
            # Initialize gradients for actor
            self.target_actor_model = self.actor_model
            self.target_critic_model = self.critic_model
...
##################################################################################################################
    ...
    def train_actor_critic(self, data):
        for d in data:
            ...
            critic_value = self.critic_model.predict(self.state2vec(cur_state))
            critic_value_future = self.critic_model.predict(self.state2vec(new_state))
            reward += self.discount_factor*future_reward

            self.actor_model.fit([self.state2vec(state), self.state2vec(action)], reward, verbose=0)
            self.critic_model.fit([self.state2vec(state), reward, verbose=0)
    ...
    Purpose:
    Returns: a neural network for the actor model and the crtiic model
    """
    ...
    def init_actor_critic(self):
        if not self.load:
            ...
            dense_1 = Dense(units=self.num_units, activation='relu')(state_inputs)
            dense_2 = Dense(units=self.num_units*2, activation='relu')(dense_1)
            dense_3 = Dense(units=self.num_units, activation='relu')(dense_2)
            outputs = Dense(1, activation='linear')(dense_3)
            # create actor
            optimizer = Adam(lr=self.lr)
            actor = Model(inputs=[state_inputs, action_inputs], output=[outputs])
            actor.compile(optimizer=optimizer, loss = "mse")
            # create critic
            critic = Model(inputs=[state_inputs], output=[outputs])
